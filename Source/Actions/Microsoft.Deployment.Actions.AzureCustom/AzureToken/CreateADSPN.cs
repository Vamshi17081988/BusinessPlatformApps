...
namespace Microsoft.Deployment.Actions.AzureCustom.AzureToken
{
    ...
    public class CreateADSPN : BaseAction
    {
        public override async Task<ActionResponse> ExecuteActionAsync(ActionRequest request)
        {
            ...
            payload.displayName = "solutiontemplateappinsights";
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                // Convert to SPN
                string spnBody = "{\"appId\":\"" + appId + "\"}";
                string graphUriBaseWithSpn = "https://graph.windows.net/{0}/servicePrincipals";
                string graphApiWithAppForSpn = string.Format(graphUriBaseWithSpn, tenantId);
                var responseSpn = await client.ExecuteGenericRequestWithHeaderAsync(HttpMethod.Post, graphApiWithAppForSpn + "?api-version=1.6", spnBody);
                
                if (responseSpn.IsSuccessStatusCode)
                {
                    var responseSpnString = await responseSpn.Content.ReadAsStringAsync();
                    JObject responseSpnObj = JsonUtility.GetJObjectFromJsonString(responseSpnString);
                    string spnObbId = responseSpnObj["objectId"].ToString();
                    responseBodyObj.Add("SPNObjectId", spnObbId);
